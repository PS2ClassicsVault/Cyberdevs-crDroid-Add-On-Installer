...
mkdir -p "$destDir"
TMPDIR=/dev/tmp
COMMONDIR=$TMPDIR/crDroidScripts
if $BOOTMODE; then
  COMMONDIR=$MODPATH/crDroidScripts
  mkdir -p "$COMMONDIR"
fi
addonDir="/sdcard/crdroidapps/addonLogs"
crDroidappsTestLog="$addonDir/logfiles/crdroid_apps.log"
newline='
'
...
echo_add_to_log() {
  echo ""
  echo "addToLog() {"
  echo "  echo \"\$(date +%Y_%m_%d_%H_%M_%S): \$1\" >> \$crDroidappsAddonLogFile"
  echo "}"
  echo ""
}
...
echo_add_file_to_log() {
  ...
  echo "    echo \"\$(date +%Y_%m_%d_%H_%M_%S): - \$output_path copied, Size: \$size\" >> \$crDroidappsAddonLogFile"
  echo "  else"
  echo "    echo \"\$(date +%Y_%m_%d_%H_%M_%S): - \$output_path does not exist\" >> \$crDroidappsAddonLogFile"
  echo "  fi"
  echo "}"
  echo ""
}
