...
if [ -z $OUTFD ]; then
  ...
  ui_print() {
    echo -e "ui_print $1\nui_print" >> /proc/self/fd/$OUTFD; test "$crDroidappsAddonLogFile" && echo "$(date +%Y_%m_%d_%H_%M_%S): $1" >> "$crDroidappsAddonLogFile";
   }
fi
...
find_config() {
  crDroidapps_config_file_name="$crDroidappsDir/crDroidapps.config"
  for location in "/tmp" "/sdcard1" "/sdcard1/NikGapps" "/sdcard" "/storage/emulated/crDroidapps" "/storage/emulated"; do
    if [ -f "$location/crDroidapps.config" ]; then
      crDroidapps_config_file_name="$location/crDroidapps.config"
      break;
    fi
  done
}
...
[ -z $crDroidapps_config_file_name ] && find_config

[ -z $execute_config ] && execute_config=$(ReadConfigValue "ExecuteBackupRestore" "$crDroidapps_config_file_name")
